...
public class RingOfEternity : Item
{
    public override void Use(CharacterState player)
    {
        if (player.isRingOfEternityCollected)
        {
            return;
        }
        AudioManager.Instance.PlayEffect("Item");
        CollectionHelper.CollectCollection("Ring of Eternity");
        player.maxHealthPoints *= 1.5f;
        player.currentHealthPoints *= 1.5f;
